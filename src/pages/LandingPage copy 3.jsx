...
export default function LandingPage() {
  ...
  const propertyType = useSelector((state) => state?.booking?.type) || null;
  console.log({ propertyType: type });

  const roomType = useSelector((state) => state?.booking?.roomType) || null;
  ...
  useEffect(() => {
    setFilters({
      ...
      city: city ? city : '',
      maxGuests: maxGuests ? maxGuests : 1,
    });
  }, [filters]);
  ...
  const filteredData = allRoomsL?.filter((item) => {
    return (
  ...
  //     ((Number(item.price) >= filters?.startPrice && Number(item.price) <= filters?.endPrice) ||
  //       '') // price range
  //   );
  // });
