...
export default function LandingPage() {
  ...
  const guestCity = useSelector((state) => state?.booking?.city?.city) || null;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [city, type, roomType, maxGuests, startPrice, endPrice]);
  ...
  function filterUserData() {
    const userData = allRoomsL?.filter((item) => {
      return (
        ...
        (item.type === type || '') &&
        (item.roomType === roomType || '')(item.maxGuests === maxGuests || 1) &&
